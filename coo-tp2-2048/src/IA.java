...
public class IA extends Thread {
	...
	/**
	 * Décision en fonction du score. On va essayer ici de maximiser le score
	 * tout en essayant de garder au maximum les blocs de grandes valeurs en bas
	 * à droite.
	 * 
	 * @param g
	 *            Grille
	...
	public void deciderScore(Grille g, Score score) {
		...
		// System.out.println(sn.scoreValue+ " " + ss.scoreValue + " "
		// +se.scoreValue+ " " + so.scoreValue);
		...
		if (ss.scoreValue >= se.scoreValue && ss.scoreValue != 0)
			d = Direction.SUD;
		else if (ss.scoreValue < se.scoreValue)
			d = Direction.EST;
		// OUEST...
		else if (so.scoreValue > 0)
			d = Direction.OUEST;
		// puis NORD en dernier recours
		else if (sn.scoreValue > 0)
			d = Direction.NORD;
		...
		// aucune fusion possible, aucun ajout de score, on teste le déplacement
		// dans l'ordre SEON
		else {
